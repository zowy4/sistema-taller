...
/**
 * E2E: Flujo post-venta (órdenes → completar → facturar)
 * Crea datos mínimos con Prisma, crea la orden vía API, cambia estados y factura.
 */
describe('Post-Venta E2E', () => {
  let app: INestApplication;
  let prisma: PrismaService;

  // Datos de prueba dinámicos para evitar colisiones por constraints únicos
  const ts = Date.now();
  const testData = {
    cliente: {
  ...
  it('debe facturar la orden y marcar entregado si hay método de pago', async () => {
    ...
    // Confirmar que la factura existe por orden
    const res2 = await request(app.getHttpServer())
    ...
    // Confirmar que la orden queda entregada
