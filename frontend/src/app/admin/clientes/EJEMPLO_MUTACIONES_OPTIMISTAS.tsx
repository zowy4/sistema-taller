/**
 * EJEMPLO COMPLETO: Página de Clientes con Mutaciones Optimistas
 * 
 * Este ejemplo muestra cómo usar useClientesMutations para:
 * - Crear clientes (UI se actualiza instantáneamente)
 * - Editar clientes (cambios aparecen al instante)
 * - Eliminar clientes (desaparecen inmediatamente)
 * 
 * Si el servidor falla, los cambios se revierten automáticamente.
 */
...
export default function ClientesPageExample() {
  ...
  // ==========================================
  // QUERY: Obtener lista de clientes
  // ==========================================
  const { 
    data: clientes = [], 
    isLoading, 
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (isEditMode && selectedCliente) {
      ...
      // ✨ CREACIÓN OPTIMISTA: El cliente aparece al instante en la tabla
    ...
    // Cerramos el modal SIN ESPERAR la respuesta del servidor
  ...
  const handleDelete = (cliente: Cliente) => {
    ...
    if (confirmed) {
      // ✨ ELIMINACIÓN OPTIMISTA: Desaparece INMEDIATAMENTE de la tabla
      // Si el servidor falla (ej: tiene órdenes activas), volverá a aparecer
      deleteMutation.mutate(cliente.id_cliente);
    }
  };

  const handleToggleActivo = (cliente: Cliente) => {
    // ✨ ACTUALIZACIÓN OPTIMISTA: El estado cambia al instante
    updateMutation.mutate({
